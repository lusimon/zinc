...
public class PersonMB extends AbstractMB implements Serializable {
	...
	public void addDogToPerson() {
		try {
			getPersonFacade().addDogToPerson(dog.getId(),
					personWithDogs.getId());
			closeDialog();
	...
	public void removeDogFromPerson() {
		try {
			getPersonFacade().removeDogFromPerson(dog.getId(),
					personWithDogs.getId());
			closeDialog();
	...
	public Person getPersonWithDogs() {
		if (personWithDogs == null) {
			...
			personWithDogs = getPersonFacade().findPersonWithAllDogs(
					person.getId());
		}
	...
	public void setPersonWithDogsForDetail(Person person) {
		personWithDogsForDetail = getPersonFacade().findPersonWithAllDogs(
				person.getId());
	}
	...
	private void reloadPersonWithDogs() {
		personWithDogs = getPersonFacade()
				.findPersonWithAllDogs(person.getId());
	}
}
